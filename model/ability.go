...
func getPriority(group string, model string, retry int) (int, error) {
	...
	err := DB.Model(&Ability{}).
		...
		Order("priority DESC").              // 按Priority降序排序
		Pluck("priority", &priorities).Error // Pluck用于将Query的结果直接扫描到一indivual切片中
	...
	if retry >= len(priorities) {
		// 如果Retrytimes数大于Priority数，则使用最小的Priority
